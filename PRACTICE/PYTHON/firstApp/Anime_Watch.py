        ...
        if ytdl_path.endswith('ytdl') or ytdl_path.endswith('ytdl.exe'):
            ...
            if not updated_already:
                ...
                if ytdl_path.endswith('ytdl'):
                    ...
                    ytdl_path)
                send_notification('Updated youtube-dl,',
...
#    final_url = ''
    ...
    logger.info("TMPFILE={0};Output-dest={1};dir_name={2};sub_name={3}"
        .format(TMPFILE, dest_dir_sub, dir_name, sub_name))
    if ytdl_path == 'default':
        youtube_dl = 'youtube-dl'
    else:
        youtube_dl = ytdl_path
    params = " --all-sub --skip-download --output "
    command = youtube_dl + params + TMPFILE + " " + url
    logger.info(command)

    yt_sub_process = QtCore.QProcess()
    yt_sub_process.started.connect(yt_sub_started)
    yt_sub_process.readyReadStandardOutput.connect(partial(yt_sub_dataReady,
    yt_sub_process))
    ...
    for i in m:
        ...
        if (i.startswith(sub_name) and i.endswith('.vtt')
            ...
            if os.path.exists(src_path):
                ...
                sub_avail = True
    if sub_avail:
        end_txt_notify = " Available\nPress Shift+J to load"
        txt_notify = "External Subtitle " + sub_ext + end_txt_notify
